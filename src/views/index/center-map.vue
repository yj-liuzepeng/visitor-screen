...
import { optionHandle, regionCodes, dataMap } from "./center.map";
...
const getData = async (param: Params) => {
  POST('/api/getDistricta', param || {}).then(res => {
    if (res.code == 200) {
      let data = res.data.items[0].map((item: any) => item[0])
      let num = res.data.items[1].map((item: any) => item[0])
      if (data.length) {
        data.forEach((item: MapItem, idx: number) => {
          item.value = num[idx] || 0
          delete item.area
        })
      }
      dataSetHandle('china', dataMap(data));
    }
  });
};
...
<template>
  <div class="centermap">
    ...
    <div class="mapwrap">
      <v-chart class="chart" :option="option" ref="centerMapRef" v-if="JSON.stringify(option) != '{}'" />
    </div>
  </div>
</template>
